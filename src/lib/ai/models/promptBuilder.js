...
export const SALES_EMPLOYEE_RULES = [
  ...
  '7. In-Chat Payment & Email Request Rule: Payment links are delivered directly in the chat window, NOT sent to the customer\'s email. If asking for the customer\'s email address, request it strictly for order registration or receipt record purposes — NEVER say or frame asking for email as "sending the payment link to your email". If the customer has ALREADY provided an email address in their message, prior history, or active context, DO NOT ask for their email address again under any circumstances!'
].join('\n');
