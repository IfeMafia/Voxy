...
import { createProduct, nairaToKobo } from "@/lib/api/products";
import {
  ArrowLeft,
  Loader2,
  AlertCircle,
...
export default function NewProductPage() {
  ...
  const effectivePrice =
    (parseFloat(form.priceNaira) || 0) - (parseFloat(form.discountNaira) || 0);
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            <div className="bg-[#0a0a0a] border border-white/[0.07] rounded-2xl p-5 sm:p-6">
              <SectionHeader
                ...
                description="The name and description Voxy uses to describe this product to customers."
              />
              <div className="space-y-4">
                ...
                <Field
                  label="Description"
                  hint="A clear description helps Voxy answer customer questions accurately."
                >
                  <textarea
                    value={form.description}
                    onChange={(e) => set("description", e.target.value)}
                    placeholder="Describe ingredients, size, what's included…"
                    rows={3}
                    className={TEXTAREA}
                  />
                </Field>
            ...
            <div className="bg-[#0a0a0a] border border-white/[0.07] rounded-2xl p-5 sm:p-6">
              <SectionHeader
                icon={Tag}
                title="Pricing"
                description="Set the selling price. Add a discount if you are running a promotion."
              />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field
                  label="Discount (₦)"
                  hint={discountPct > 0 ? `${discountPct}% off — customer pays ₦${effectivePrice.toLocaleString()}` : "Optional promotional discount."}
                >
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-zinc-500 font-medium">₦</span>
                    <input
                      ...
                      onChange={(e) => set("discountNaira", e.target.value)}
                      placeholder="0.00"
                      className={INPUT + " pl-7"}
                    />
                  </div>
            ...
            <div className="bg-[#0a0a0a] border border-white/[0.07] rounded-2xl p-5 sm:p-6">
              <SectionHeader
                icon={Package}
                title="Inventory"
                description="Stock tracking helps Voxy avoid selling items that are out of stock."
              />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field
                  label="Stock quantity"
                  hint="Leave blank if you have unlimited stock (e.g. a service)."
                >
                  <input
                    type="number"
                    min="0"
                ...
                <Field
                  label="Tags"
                  hint="Comma-separated. Helps Voxy categorise and recommend this item."
                >
                  <input
                    value={form.tags}
                    onChange={(e) => set("tags", e.target.value)}
                    placeholder="e.g. food, lunch, spicy"
                    className={INPUT}
                  />
                </Field>
              </div>
            ...
            {/* Section 4: Advanced (progressive disclosure) */}
              ...
              {showAdvanced && (
                <div className="px-5 sm:px-6 pb-6 space-y-4 border-t border-white/[0.05]">
                  <div className="pt-4">
                    <Field
                      label="Product Image"
                      hint="Upload product photo to Cloudinary. Voxy will present this image directly to buyers."
                    >
                      <ImageUploadDropzone
                        value={form.imageUrl}
                  ...
                  <Field
                    label="Voxy sales note"
                    hint="Private guidance for how Voxy should present or pitch this item. Customers don't see this."
                  >
                    <textarea
                      value={form.voxyNote}
                      onChange={(e) => set("voxyNote", e.target.value)}
                      placeholder="e.g. Emphasise freshness. Upsell the drink combo."
                      rows={2}
                      className={TEXTAREA}
                    />
                  </Field>
          ...
          <div className="w-full lg:w-64 xl:w-72 shrink-0 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[11px] text-zinc-600 mt-0.5">
                    {form.isAvailable
                      ? "Customers can see and order this item via Voxy."
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full border-2 transition-colors focus:outline-none ${
                    form.isAvailable
                      ? "bg-[#00D18F] border-[#00D18F]"
            ...
            <div className="space-y-2">
              <button
                ...
                disabled={saving}
                className="w-full h-10 bg-[#00D18F] text-black text-sm font-semibold rounded-xl hover:bg-[#00D18F]/90 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
              >
              ...
              <Link
                href="/business/products"
                className="w-full h-10 border border-white/[0.08] text-zinc-400 text-sm font-medium rounded-xl hover:text-white hover:border-white/[0.15] transition-colors flex items-center justify-center"
              >
                Cancel
              </Link>
            </div>
